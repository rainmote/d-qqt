(ns d-qqt.core
    (:require [d-qqt.utils :as utils]
      ...
      #?(:clj  [d-qqt.macros :refer [read-file]]
         :cljs [d-qqt.macros :refer-macros [read-file]])))
...
(defn init [game]
      ...
      (let [name "maomao"
            character-process (fn [v]
                                  ...
                                  (let [metas (character/load v)]))
            get-character-fn (fn [player]
                                 (let [p (utils/format "object/frame/character/%s.edn" (:character player))]
                                      (utils/read-edn p character-process)))]
            (utils/read-edn (utils/format "object/player/%s.edn" name)
                             (fn [v]
